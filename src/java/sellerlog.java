...
public class sellerlog extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet sellerlog</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet sellerlog at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       HttpSession session1=request.getSession();
        ...
        try(PrintWriter out = response.getWriter())
        {
            ...
            rs=st.executeQuery("select * from seller where username='"+Username+"' and password='"+Password+"'");
            if(rs.next())
            {
